...
public class NodeUtil {
	...
	public static <T extends Comparable<T>> BinaryNode<T> toBinaryNode (Node<T> node) {
	...
	public static <T extends Comparable<T>> boolean nodeIsBS (BinaryNode<T> node) {
		boolean ret = (node.getLeft()==null || node.getLeft().getValue().compareTo(node.getValue())<0) &&
